...
+ **2026-09-11** v0.2.1 — base skills (`skill-creator`, `aidrin`) installed from their upstream repositories at init; the readiness gate wraps every `aidrin` call with `dsagt-run`; the server opens only the project KB; packaging extras (`traces`, `kb`, `cli`, `all`); Intel Mac support dropped
...
- `mcp>=2.0,<3.0` — MCP server framework (core)
- `mlflow>=3.11,<4.0` — trace store and observability, serverless SQLite backend (`traces` extra)
- `chromadb>=1.5.1` — vector store (`kb` extra)
- `sentence-transformers>=6.0,<7.0` — local embeddings and reranking (`kb` extra)
- `llama-index-core>=0.11` — document and code chunking (`kb` extra)
- `rank-bm25>=0.2.2` — sparse keyword retrieval for hybrid search (`kb` extra)
- `questionary>=2.0` — interactive `dsagt init` menus (`cli` extra)
...
```bash
# For use:
pip install "dsagt[all] @ git+https://github.com/AI-ModCon/dsagt.git"
