...
- `DESIGN.md`: present architecture and the module map.
- `DEVELOPMENT.md`: open work.
- `docs/`: the MkDocs site. `docs/developer.md` is the contributor guide (setup, tests, lint, docs build, packaging extras).
- `history/decisions.md`: settled decisions. `history/parked/`: removed uncommitted files. Nothing there describes present state.
...
- **trace**: one session's spans as plain data (`traces.trace.Trace`). The **heartbeat** (`mcp.server._heartbeat`) runs `traces.collector.TraceCollector`; the **deferred final turn** is the open last turn a periodic pass withholds; **catch-up** re-collects the previous session at startup (`session.catch_up_extraction`).
...
- Modules on the `dsagt-run` path import no heavy dependency at module scope, and every subpackage imports on a core-only install.
