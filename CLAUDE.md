...
- `skills/` — External skill-catalog data plane (`SkillsCatalog`: clone/sync/index/install), the `SkillRouter` render facade, the Genesis-derived keyword scorer (`rank_skills`), and the base skills (`BASE_SKILLS` / `install_base_skills`: `skill-creator` from the genesis catalog's `basedata-skills/`, `aidrin` from the AIDRIN repository) that every `dsagt init` installs into `<project>/skills/`. The package holds no skill directories.
