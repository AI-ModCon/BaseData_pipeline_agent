...
- `readiness.py` — The opt-in readiness gate (`dsagt init --readiness aidrin`): `ensure_aidrin` (one-time shared venv install under `~/dsagt-projects/.tools/aidrin/`), `PROFILES` (the metric sets the gate instructions name), and `instructions_block` (appended to the agent's instructions file by `static_agent_record`; it points the agent at the `aidrin` base skill and the installed executable). The gate adds no code to the project.
- `skills.py` — External skill-catalog data plane (`SkillsCatalog`: clone/sync/index/install), the `SkillRouter` render facade, the Genesis-derived keyword scorer (`rank_skills`), and the base skills (`BASE_SKILLS` / `install_base_skills`: `skill-creator` from the genesis catalog's `basedata-skills/`, `aidrin` from the AIDRIN repository) that every `dsagt init` installs into `<project>/skills/`. The package holds no skill directories.
