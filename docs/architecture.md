...
AIDRIN as the check code around every tabular pipeline stage. Enabling the gate at `dsagt init` installs AIDRIN once, copies the bundled `aidrin` code into the project, and appends the gate rules to the agent's instructions; the agent runs a fixed metric profile before and after each stage and reports the delta.
