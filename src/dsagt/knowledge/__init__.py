...
if TYPE_CHECKING:
    # Annotation-only (``from __future__ import annotations`` defers them).
    # numpy is a kb-extra dependency; a runtime module-scope import would make
    # ``import dsagt.knowledge`` — and everything that imports it (memory,
    # session, the MCP server) — fail on a core-only install.
    import numpy as np
...
class LocalEmbedder(Embedder):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError("local embeddings need `dsagt[kb]`") from e
...
class APIEmbedder(Embedder):
    ...
    def embed(self, texts: list[str]) -> np.ndarray:
        try:
            ...
            raise ImportError("the knowledge base needs `dsagt[kb]`") from e
...
class KnowledgeBase:
    ...
    def _rerank(self, query: str, results: list[dict], top_k: int) -> list[dict]:
        if self._reranker is None:
            try:
                ...
                raise ImportError("reranking needs `dsagt[kb]`") from e
    ...
    def _chunk_file(self, path: Path, collection: str) -> Iterator[dict]:
        ...
        try:
            ...
            raise ImportError("document ingestion needs `dsagt[kb]`") from e
