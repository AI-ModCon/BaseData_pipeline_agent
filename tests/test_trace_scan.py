...
def test_ack_dir_overrides_where_ack_files_land(tmp_path):
    """A downstream application keeps trace state beside its own state dir."""
    trace = Trace("t", "p:s", "claude", "p")
    trace.add_agent_root("r1", "c", start_time=None, prompt="")
    rec = _Recorder("rec")
    collector = TraceCollector(
...
def test_make_trace_collector_sessions_root(tmp_path):
    """``sessions_root`` reaches the codex/cline reader, so a downstream
    consumer watching a hand-started session (rollouts under
    ``~/.codex/sessions``, not the project's ``.codex-data``) can use the
    factory instead of wiring the reader itself."""
